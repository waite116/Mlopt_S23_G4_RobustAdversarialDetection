...
def load_dicts():
    ...
    D_A_FILENAME = dict_path+'D_a.npy'
    D_S_FILENAME = dict_path+'D_s.npy'
    D_a = np.load(D_A_FILENAME)
    D_s = np.load(D_S_FILENAME)
    return (D_s, D_a)
""" 
...
# solve optimization problem  (SCS)  max 50 iters to generate new c_s and c_a
def objective(x_prime, D_s, D_a, cs_s, ca_s, lamb_s, lamb_a, T_s, T_a):
  
  t1s = []
  #np.zeros(x_prime.shape)
  t2s = []
  #np.zeros(x_prime.shape)
  # difference term
  for i in T_s:
...
def recover_sparse(x_prime,D_a, D_s, gamma=0.9): 
    ...
    while not done:
        ...
        #C_a = cp.Variable(shape=c_a.shape)
        #C_s = cp.Variable(shape=c_s.shape)
...
# Reformat into numpy arrays for easy saving
...
""" 
Robust Detection for Mixed PGD attacks
"""
def detect_robust_mixed_PGD(network, device, D_s, D_a, subset_size=1000, num_attacks=300,eps_l1 = 10., eps_l2 =2., eps_linf = .3):
    ...
    for i in range(len(X_perturbed)):
        ...
        #print("Attack: {}, Real {}, f(x)={}, f(x_hat) = {}, f(x_hat_null)={}".format(attack_type, y_small[i], pred, pred_x_hat,pred_x_hat_null))
...
""" 
Robust Detection for Single Type attacks
"""
